pipeline {
	...
	post{
		always{
			script {
				env.TIMESTAMP = bat(script:'timestamp.bat', returnStdout: true).trim()
				
				if (currentBuild.currentResult == 'FAILURE') {
					emailext body: 'Check console output at $BUILD_URL to view the results. \n\n ${CHANGES} \n\n -------------------------------------------------- \n${BUILD_LOG, maxLines=100, escapeHtml=false}', 
                    to: "${EMAIL_TO}", 
                    subject: 'QSTesting Build #$BUILD_NUMBER FAILURE in Jenkins: SSC_'+echo $TIMESTAMP
				}
				else {
					emailext body: 'Attach test result report here.', 
                    to: "${EMAIL_TO}", 
                    subject: 'QSTesting Build #$BUILD_NUMBER SUCCESS in Jenkins: SSC_'+echo $TIMESTAMP
				}
			}
